...
#define STM32F4_CCMRAM_BASE          0x10000000UL
#define STM32F4_CCMRAM_SIZE          0x00010000UL
#define FATFS_DMA_SCRATCH_SIZE       512U
...
static uint32_t fatfs_dma_scratch[FATFS_DMA_SCRATCH_SIZE / sizeof(uint32_t)];
...
static bool buffer_overlaps_ccm(const void * buf, uint32_t size)
{
    uintptr_t start = (uintptr_t)buf;
    uintptr_t end;
    uintptr_t ccm_start = STM32F4_CCMRAM_BASE;
    uintptr_t ccm_end = STM32F4_CCMRAM_BASE + STM32F4_CCMRAM_SIZE - 1U;

    if(size == 0U) return false;
    end = start + (uintptr_t)size - 1U;
    if(end < start) return true;

    return (start <= ccm_end) && (end >= ccm_start);
}
...
static lv_fs_res_t fs_read(lv_fs_drv_t * drv, void * file_p, void * buf, uint32_t btr, uint32_t * br)
{
    ...
    if(!buffer_overlaps_ccm(buf, btr)) {
        UINT bytes_read = 0;
        res = f_read(file_p, buf, btr, &bytes_read);
        if(br) *br = bytes_read;
    ...
    while(total_read < btr) {
        uint32_t remain = btr - total_read;
        UINT chunk = (remain > FATFS_DMA_SCRATCH_SIZE) ? FATFS_DMA_SCRATCH_SIZE : (UINT)remain;
        UINT chunk_read = 0;

        res = f_read(file_p, fatfs_dma_scratch, chunk, &chunk_read);
        if(res != FR_OK) {
            if(br) *br = total_read + chunk_read;
            return LV_FS_RES_UNKNOWN;
        }

        lv_memcpy(dst + total_read, fatfs_dma_scratch, chunk_read);
